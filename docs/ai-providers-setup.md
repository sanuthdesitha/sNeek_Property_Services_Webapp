...
5. Confirm the **Social post composer** shows `ollama`, then generate a draft. `AI_TEXT_PROVIDER` independently selects its provider. Run `ollama ps` inside the container to inspect model/processor use.
