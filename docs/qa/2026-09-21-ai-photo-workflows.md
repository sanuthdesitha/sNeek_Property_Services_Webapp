...
2. For comparison and vision fallback, select OpenAI, Anthropic or local Ollama and configure the selected provider on both web and worker. See [provider connection and Hostinger setup](../ai-providers-setup.md). Older saved settings retain Anthropic; new settings default to OpenAI with features disabled. Ollama never falls back to a paid provider. Use the admin connection check to verify model access; configuration presence alone is not a connection test.
3. For dedicated classifier training (optional; provider-only photo analysis does not require this service), deploy `services/photo-recognition` with its persistent model volume and a compatible local OpenCLIP checkpoint. Supply a shared `MODEL_SERVICE_TOKEN` of at least 24 characters, then set the app's private `MODEL_SERVICE_URL` and matching token. The service README contains the HTTP contract, CPU/GPU requirements and storage rules. No checkpoint downloads occur during app requests. Provisioning weights and building the Docker image remain deployment steps.
