...
async function loadTodayStops(
  userId: string
): Promise<{ stops: Stop[]; isoDate: string }> {
  ...
  const dayStart = new Date(
    Date.UTC(y, zonedNow.getMonth(), zonedNow.getDate(), 0, 0, 0)
  );
  ...
  const stops: Stop[] = assignments
    ...
    .map((a) => ({
      ...
      enRouteStartedAt: a.job!.enRouteStartedAt
        ? a.job!.enRouteStartedAt.toISOString()
        : null,
...
export default async function CleanerRoutePage() {
  let session;
  try {
    ...
    redirect("/login?callbackUrl=/cleaner/route");
  }
