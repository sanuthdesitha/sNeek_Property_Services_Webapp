...
function sanitizeBankDetails(value: unknown): BankDetails | null {
  ...
  return { accountName, bankName, bsb, accountNumber };
}
...
export async function upsertUserExtendedProfile(
  ...
  if (store.profiles.length > 2000) store.profiles = store.profiles.slice(-2000);
