...
import { inboxStateSchema, nextInboxState, type InboxState, type InboxMutation } from "@/lib/notifications/inbox-state";

type Row = { id: string; subject: string | null; body: string; createdAt: string; href: string; isRead?: boolean; canMarkRead?: boolean; inboxState?: InboxState };
...
function SessionInbox({ accent }: { accent: string }) {
  ...
  const changeState = async (row: Row, action: InboxMutation["action"]) => {
    ...
    try {
      const response = await fetch("/api/notifications/inbox-state", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: row.id, revision: row.inboxState.revision, action }) });
      if (!response.ok) throw new Error();
      const result = inboxStateSchema.parse((await response.json()).state);
      const expected = nextInboxState(row.inboxState, action);
      if (result.revision !== expected.revision || result.followUp !== expected.followUp || result.archived !== expected.archived) throw new Error();
  ...
  const markRead = async (id: string) => {
    ...
    try {
      ...
      setRows(previous => previous.map(row => row.id === id ? { ...row, isRead: true } : row));
      ...
      try {
        ...
        if (!disposed) {
          const items = page.items.map(row => {
            ...
            return { ...row, ...(confirmedReadIds.current.has(row.id) ? { isRead: true } : {}),
        ...
        <Dialog.Content data-skin="estate" data-portal-accent={accent}
          ...
          <label className="mx-4 mb-3 text-sm">Personal follow-up (loaded history)
            <select aria-label="Personal follow-up filter" className="ml-2 min-h-11 rounded border bg-transparent" value={followUpFilter} onChange={event => setFollowUpFilter(event.target.value)}>
              <option value="ACTIVE">Inbox</option><option value="NEEDS_ACTION">Needs action</option><option value="RESOLVED">Resolved</option><option value="ARCHIVED">Archived</option>
            </select>
          </label>
          <p className="mx-4 mb-3 text-xs">Follow-up and archive affect only your inbox. They do not change the job or confirm delivery.</p>
          <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 pb-6" aria-busy={loading}>
            ...
            {!loading && !error && !filtered.length ? <p role="status" className="py-3 text-sm">{query.trim() || unreadOnly || followUpFilter !== "ACTIVE" ? "No matching notifications in loaded history." : nextCursor ? "No visible notifications loaded." : "No recent notifications."}</p> : null}
