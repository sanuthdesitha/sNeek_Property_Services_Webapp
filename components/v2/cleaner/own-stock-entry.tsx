...
export function OwnStockEntry({ items, onSaved, scope, holding }: { holding?: { id: string; quantity: number; updatedAt: string }; scope: string; items: StockItem[]; onSaved: () => Promise<void> }) {
  ...
  const storageKey = `cleaner-held-stock:${scope}${holding ? `:adjust:${holding.id}` : ""}`;
  ...
  async function save() {
    ...
    try {
      const response = await fetch("/api/cleaner/inventory/held-stock", { method: holding ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(entry) });
