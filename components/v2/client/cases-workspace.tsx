 ...
 *   POST  /api/client/cases                   { title?, description, caseType }
 *   GET   /api/client/cases/[id]              → CaseView (with comments)
 *   PATCH /api/client/cases/[id]              { comment } | { status: "OPEN" }
 * Styled purely with `--e-*` tokens. No v1 UI imports.
 */
import { useCallback, useEffect, useMemo, useState } from "react";
import { format } from "date-fns";
import { ArrowLeft, Loader2, MessageSquarePlus, Plus, RotateCw, Send } from "lucide-react";
import {
  EBadge,
  EButton,
  ECard,
...
const CASE_TYPES: { value: string; label: string }[] = [
  { value: "CLIENT_DISPUTE", label: "Service issue / dispute" },
