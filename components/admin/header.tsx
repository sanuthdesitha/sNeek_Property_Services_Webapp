...
export function AdminHeader({ title, companyName = "sNeek Property Services", logoUrl = "" }: AdminHeaderProps) {
  ...
  return (
    <>
      <header
        className={`sticky top-0 z-40 shrink-0 border-b border-border/70 bg-background/75 px-3 py-3 backdrop-blur-md transition-transform duration-300 sm:px-4 md:px-6 ${
      ...
      >
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex min-w-0 items-center gap-3">
            ...
            {logoUrl ? (
              <img src={logoUrl} alt={`${companyName} logo`} className="h-9 w-9 rounded-md bg-surface p-0.5 object-cover shadow-sm dark:bg-surface-raised" />
            ) : (
          ...
          <div className="ml-auto flex w-full flex-wrap items-center justify-end gap-2 md:w-auto md:gap-3">
            <div className="hidden items-center gap-2 rounded-full border border-border/70 bg-surface/75 px-3 py-1.5 text-xs font-medium text-muted-foreground shadow-sm lg:flex">
                ...
                <div className="max-h-[220px] space-y-1.5 overflow-auto">
                  {notifications.slice(0, 20).map((item) => (
                    <Link
                      ...
                      className="block rounded-xl border border-border/60 bg-muted/30 p-2.5 transition-colors hover:bg-muted/60 dark:bg-muted/20 dark:hover:bg-muted/40"
                    >
      ...
      <Dialog open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
        <DialogContent className="flex max-h-[85vh] max-w-sm flex-col overflow-hidden p-0">
          ...
          <div className="border-t border-border/70 px-3 py-3">
            <div className="mb-3 flex items-center gap-3 rounded-xl border border-border/70 bg-surface/70 px-3 py-3">
              {session?.user?.image ? (
        ...
        {active.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              onClick={onNavigate}
              className="flex items-center gap-3 rounded-xl border border-amber-200/60 bg-amber-50/60 px-3 py-2.5 transition-colors hover:bg-amber-100/60 dark:border-amber-500/30 dark:bg-amber-500/10 dark:hover:bg-amber-500/20"
            >
