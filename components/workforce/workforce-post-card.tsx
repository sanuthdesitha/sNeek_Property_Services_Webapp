...
export function WorkforcePostCard({
  ...
  return (
    <article className={`overflow-hidden rounded-[28px] border border-border bg-surface shadow-[0_18px_46px_rgba(15,23,42,0.08)] dark:shadow-none ${className}`}>
      <div className="flex items-center justify-between gap-3 border-b border-border px-4 py-3">
        <div className="flex min-w-0 items-center gap-3">
          {post.createdBy?.image ? (
            <img src={post.createdBy.image} alt={authorName} className="h-11 w-11 rounded-full object-cover ring-2 ring-surface shadow-sm" />
          ) : (
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-muted-foreground">{formatPostDate(post.createdAt)}</p>
          </div>
        </div>
      ...
      {post.coverImageUrl ? (
        <div className="bg-[radial-gradient(circle_at_top_left,rgba(255,213,128,0.28),transparent_38%),radial-gradient(circle_at_bottom_right,rgba(45,212,191,0.28),transparent_36%)] px-4 pt-4">
          <div className="overflow-hidden rounded-[24px] border border-border bg-muted shadow-inner dark:shadow-none">
            <img
      ...
      <div className="space-y-3 px-4 pb-5 pt-4">
        <div>
          ...
          <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-foreground/80">{post.body}</p>
        </div>
            ...
            {post.attachments!.map((attachment) => {
              ...
              return (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="overflow-hidden rounded-2xl border border-border bg-surface-raised transition hover:border-primary/40 hover:bg-primary/5"
                >
                  {isImage ? (
                    <img
                      src={attachment.url}
                  ...
                  <div className="p-3">
                    <p className="line-clamp-1 text-sm font-medium text-foreground">{attachment.label || attachment.fileName || "Attachment"}</p>
                    <p className="mt-1 text-xs text-muted-foreground">{isImage ? "Open image" : attachment.mimeType || "Open file"}</p>
                  </div>
                </a>
              );
            })}
        ...
        {typeof post.seenCount === "number" ? (
          <p className="text-xs text-muted-foreground">Seen by {post.seenCount} staff</p>
        ) : null}
      </div>
    </article>
  );
