...
export async function recordOwnHeldStock(holderUserId: string, raw: unknown, actorUserId = holderUserId, database?: Prisma.TransactionClient) {
...
export async function adjustOwnHeldStock(holderUserId: string, raw: unknown, actorUserId = holderUserId, administrative = false) {
  const input = adjustOwnHeldStockSchema.parse(raw);
  const receiptId = `held_adjust_${createHash("sha256").update(JSON.stringify([holderUserId, actorUserId, input.requestId, ...(administrative ? ["admin"] : [])])).digest("hex")}`;
  return db.$transaction(async tx => {
    ...
    await tx.auditLog.create({ data: { id: receiptId, userId: actorUserId, action: administrative ? "HELD_STOCK_ADMIN_ADJUSTED" : "HELD_STOCK_SELF_ADJUSTED", entity: "HeldStock", entityId: held.id, before: { quantity: held.quantity, updatedAt: held.updatedAt.toISOString() }, after } });
    return { id: held.id, duplicated: false };
  });
}

/** Role authorization belongs to the admin route; holder is read from the ledger. */
export async function adjustAdminHeldStock(actorUserId: string, raw: unknown) {
  const input = adjustOwnHeldStockSchema.parse(raw);
  const held = await db.heldStock.findUnique({ where: { id: input.heldStockId }, select: { holderUserId: true } });
  if (!held) throw new HeldStockEntryError(404, "Holding not found.");
  return adjustOwnHeldStock(held.holderUserId, input, actorUserId, true);
}